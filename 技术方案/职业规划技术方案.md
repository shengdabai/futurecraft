...
| Game | Soul Scan、Simulation、Tutor、Boss Battle、Life Simulation | Postgres |
...
| Life Simulation | 人生模拟视频脚本生成、变体生成 | Postgres + AI Service |
...
| **AI 人生模拟** | 根据用户描述生成不同人生体验的视频脚本 | Gemini 2.5 Flash |
...
**核心概念 (Core Concept)**
- 项目名称：AI 人生模拟 (AI Life Simulation)
- 核心原则：根据用户想要什么样的版本的人生，来生成相关的视频内容

**运行机制 (Principle)**
1. **用户需求输入**：用户定义自己想要体验的"人生版本"或特定身份
2. **系统响应**：AI 根据用户的描述，定向生成相关的模拟视频脚本内容
3. **差异化体验**：通过不同的描述参数，生成完全不同的视觉模拟体验，让用户身临其境
...
**AI 执行流程**：
1. AI 解析用户的具体描述
2. 生成第一视角的"教师生活"视频脚本（3-5个场景）
3. 每个场景包含：场景设置、动作描述、对话/独白、情感基调、时长
4. 如果用户调整描述（例如"乡村教师"或"大学教授"），AI 将生成不同的模拟体验

#### 3.4.3 技术实现

**前端组件**：`LifeSimulationView.tsx`
```typescript
interface LifeSimulationRequest {
  description: string;      // 用户描述（如"乡村教师"）
  language?: 'zh' | 'en';   // 语言
  duration?: 'short' | 'medium' | 'long';  // 视频时长
}
...
**后端服务**：`lifeSimulationService.ts`
- `generateLifeSimulationVideo()`：生成单个视频脚本
- `generateLifeSimulationVariations()`：生成多个变体版本
- `getLifeSimulationSuggestions()`：获取热门人生模拟建议

**API 端点**：
- `POST /life-simulation/generate`：生成视频脚本
- `POST /life-simulation/variations`：生成多个变体
- `GET /life-simulation/suggestions`：获取建议列表

#### 3.4.4 UI/UX 特性

1. **输入界面**：
   - 自由文本输入框（最多500字符）
   - 热门推荐标签（乡村教师、大学教授、软件工程师、咖啡师、医生等）
   - 视频时长选择（短/中/长）

2. **结果展示**：
   - 视频脚本标题和描述
   - 场景卡片列表（包含场景设置、动作、对话、情感基调）
   - 分类标签
   - 收藏/重新生成功能
...
| 接口 | 方法 | 说明 | 分页 |
|---|---|---|---|
| `/life-simulation/generate` | POST | 生成人生模拟视频脚本 | - |
| `/life-simulation/variations` | POST | 生成多个变体版本 | - |
| `/life-simulation/suggestions` | GET | 获取热门建议 | - |

**请求示例 (POST /life-simulation/generate)**：
```json
{
  "description": "我想体验一下当老师的生活",
  "language": "zh",
  "duration": "medium"
}
```
...
{
  "title": "乡村教师的一天",
  "description": "体验乡村教师的辛勤与温馨",
  "scenes": [
    {
      "sceneNumber": 1,
      "setting": "清晨的教室，阳光透过窗户",
      "action": "我早早来到教室，整理课本和教案...",
      "dialogue": "又是新的一天，希望孩子们都能有所收获",
      "mood": "peaceful",
      "duration": "20s"
    }
  ],
...
| Week 9 | AI Life Simulation: 视频脚本生成 + 前端界面 |
| Week 10-11 | IAP: 订阅 + 积分包 + Credits + Notifications V2 |
| Week 12-13 | 测试 + Bug Fix + 合规文档 + App Store 提审 |
